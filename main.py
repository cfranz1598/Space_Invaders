"""
    ...
    Splash Screen ==> Main Menu. You then have four choices to either  GamePlay, ModifyParameters, 
    ShowCredits, Quit.  GamePlay goes to GameStart and into the actual game. That takes you to either
    LevelComplete or GameEnd when you finally die.  From there you can restart the game, go back to 
    the menu, or end the program.
...
# these are available states for the Spave Invaders game system.
states = {
    "SPLASH": Splash(game_assets),   # Splash Screen
    "MENU": Menu(game_assets),  # Main Menu
    "SOUND": Sound(game_assets),  # game parms mod
    "CREDITS": Credits(game_assets),  # display game etc. credits
    "GAMEPLAY": GamePlay(game_assets),  # Game Play
    "GAMESTART": GameStart(game_assets),  # Game start screen
    "GAMELEVEL": GameIntermission(game_assets),  # Game between levels break
    "GAMEOVER": GameOver(game_assets),  # Game over screen
}
